...
test_cases[home_value < mortgage_balance, mortgage_balance := home_value * 0.9]
...
# plot results 
library(ggplot2)
plot <- out[employment_income == 200000 & 
                 home_value == 1000000 &
...
plot
# plot 1
ggplot(plot, aes(x = mortgage_balance, y = sm_effective_value)) + 
  geom_point() +
  geom_line()
